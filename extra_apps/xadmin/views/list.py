...
from costtrendreport.models import CostHourTrendReport, CostDailyTrendReport
    ...
    def get_list_queryset(self):
        ...
        if self.user.is_superuser != 1:
            ...
            if not queryset is None:
                if str(queryset).find('CostDailyTrendReport') > 0:
                    qs = CostDailyTrendReport.objects.filter(adUser_id=self.user.id).extra(select={
                        'cost': 'CONVERT(cost / deliveryRate, DECIMAL(10,2))',
                    })
                    # Then, set queryset ordering.
                    queryset = qs.order_by(*self.get_ordering())

                    # Return the queryset.
                    return queryset
                if str(queryset).find('CostHourTrendReport') > 0:
                    qs = CostHourTrendReport.objects.filter(adUser_id=self.user.id).extra(select={
                        'cost': 'CONVERT(cost / deliveryRate, DECIMAL(10,2))',
                    })
                    # Then, set queryset ordering.
                    queryset = qs.order_by(*self.get_ordering())
